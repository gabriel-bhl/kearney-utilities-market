...
def thread_read_excel(paths: list, import_result: list, nome_thread: str, cor: str):
    """
    Esta função é especifica para aplicação em uma multithread para acelerar a importação

    :param paths: Lista de paths para importação
    :param import_result: Onde os arquivos importados são salvos
    :param nome_thread: Nome da Thread
    :param cor: Cor da Thread
    :return:
    """
    for path in tqdm(paths, desc=nome_thread, colour=cor, position=0):
        import_result.append(pd.read_excel(path))
    return


def rename_columns(columns: list):
    """
    Transformação do nome das colunas para o padrão

    :param columns: lista contendo os nomes das colunas
    :return: lista de nomes corrigidos
    """

    # Dicionário de transformações
    colunas_renomeadas = {
        'Centr': 'centro',
        'Data': 'dt',
        'Hora': 'hora',
        'Organização de Vendas': 'organizacao_vendas',
        'Numero do Contrato:': 'n_contrato',
        'Tipo de Contrato:': 'tipo_contrato',
        'Contrato Cancela Sub': 'contrato_cancela_sub',
        'Pago na data': 'pago_dt',
        'Número PO:': 'n_po',
        'Ordem': 'ordem',
        'Status do Contrato:': 'status_contrato',
        'Num Client': 'n_cliente',
        'Nom Cliente': 'nome_cliente',
        'Modalidade': 'modalidade',
        'Distrito de Vendas:': 'distrito_vendas',
        'Filial': 'filial',
        'Supervisor': 'supervisor',
        'Agente': 'agente',
        'Data Vencimento': 'dt_vencimento',
        'Data Credito': 'dt_credito',
        'Data inicial': 'dt_inicial',
        'Data final': 'dt_final',
        'Data criação': 'dt_criacao',
        'DtDivRemes': 'dt_div_remes',
        'Estado': 'estado',
        'Cidade': 'cidade',
        'Bairro': 'bairro',
        'Produto': 'produto',
        'Descrição': 'descricao',
        'QtdProgramada': 'qtd_programada_1',
        'Qtdremetida': 'qtd_remetida_1',
        'QtdPendente': 'qtd_pendente_1',
        'Vlr.Unitário': 'valor_unitario_reais_1',
        'ValoremReais': 'valor_reais_1',
        'TaxadoDolar': 'taxa_dolar_1',
        'ValoremDolar': 'valor_dolar_1',
        'Comissão': 'comissao',
        'Desc.Concedido(%)': 'desconto_concedido_1',
        'Tipo de embalagem:': 'tipo_embalagem',
        'Cultura': 'cultura',
        'Bl Rem': 'bloqueio_rem',
        'Bl pgto': 'bloqueio_pagamento',
        'Bl Cred': 'bloqueio_credito',
        'Motivo de Recusa': 'motivo_recusa',
        ...
        'Cód. Postal': 'codigo_postal',
        'Endereço': 'endereco',
        'Recebedor': 'recebedor',
        'Moeda': 'moeda',
        'Venda Compartilhada': 'venda_compartilhada',
        'Centro descr': 'centro_descr',
        'Liberação': 'liberacao',
        'Razão do bloqueio de crédito': 'razao_bloqueio_credito',
        'Data do bloqueio': 'dt_bloqueio',
        'Pedido do cliente': 'pedido_cliente',
        'Item Pedido do cliente': 'item_pedido_cliente',
        'ItemdaOrdem': 'item_ordem_1',
        'TaxaDólarparaFaturamento': 'taxa_dolar_faturamento_1',
        'ValorUnitR$paraFaturamento': 'valor_unitario_reais_faturamento_1',
        'Domicílio Fiscal': 'domicilio_fiscal',
        'Bloq de entrega': 'bloqueio_entrega',
        'St Faturamento': 'status_faturamento_1',
        'Doc Incompleto': 'doc_incompleto',
        'Nro. Cotação:': 'n_cotacao',
        'Contrato SAP Cancela Sub': 'contrato_sap_cancela_sub',
        'Contrato Master': 'contrato_master',
        'QtdeBloqueadaEntrega': 'qtd_bloqueada_entrega_1',
        'Versão Lista Preço': 'versao_lista_preco',
        'Data Fixação Preço': 'dt_fixacao_preco',
        'Margem em Dólar (ZMAD)': 'margem_dolar_zmad_1',
        'Item da Ordem': 'item_ordem_2',
        'Qtd Programada': 'qtd_programada_2',
        'Qtd remetida': 'qtd_remetida_2',
        'Qtd Pendente': 'qtd_pendente_2',
        'Valor em Reais': 'valor_reais_2',
        'Taxa do Dolar': 'taxa_dolar_2',
        'Valor em Dolar': 'valor_dolar_2',
        'Taxa Dólar para Faturamento': 'taxa_dolar_faturamento_2',
        'Valor Unit R$ para Faturamento': 'valor_unitario_reais_faturamento_2',
        'Unnamed: 71': 'status_faturamento_2',
        'Vlr,Unitário': 'valor_unitario_2',
        'Desc, Concedido (%)': 'desc_concedido_2',
        'Desc. Concedido (%)': 'desc_concedido_3',
        'Qtde Bloqueada Entrega': 'qtd_bloqueada_entrega_2',
        'MargememDólar(ZMAD)': 'margem_dolar_zmad_2',
        'Unnamed: 77': 'unnamed_77',
        'Unnamed: 78': 'unnamed_78',
        'Unnamed: 79': 'unnamed_79',
        'Unnamed: 80': 'unnamed_80',
        'Frete Estatísti': 'frete_estatistico',
        'Direito Fiscal do ICMS': 'direito_fiscal_icms',
        'Direito fiscal do PIS': 'direito_fiscal_pis',
        'Direito fiscal do COFINS': 'direito_fiscal_cofins',
        'Direito fiscal  do IPI': 'direito_fiscal_ipi',
        'Item Contrato': 'item_contrato'
    }

    # ‘Loop’ de substituição
    colunas_arrumadas = []
    for col in columns:
        if col in colunas_renomeadas.keys():
            colunas_arrumadas.append(colunas_renomeadas[col])
        else:
            # Arrumando colunas que não constam no dicionário
            print('coluna sem correspondente: {}'.format(col))
            coluna_arrumada = und(col).lower().replace(' ', '_')
            coluna_arrumada = ''.join([a for a in coluna_arrumada if (a.isalnum()) | (a == '_')])
            colunas_arrumadas.append(coluna_arrumada)

    return colunas_arrumadas
...
def arrumar_datas(df: pd.DataFrame, colunas_data: list):
    for column in colunas_data:
        m = df[column].astype(str).str.isdigit()
        df.loc[m, column] = df.loc[m, column].astype(int).apply(from_excel_ordinal)
        df[column] = pd.to_datetime(df[column], errors='coerce')

    return df[colunas_data].info()


def converter_datas_excel(df: pd.DataFrame, coluna_data: str):
    df['data_arrumada'] = pd.TimedeltaIndex(df[coluna_data], unit='d') + dt.datetime(1899, 12, 30)
